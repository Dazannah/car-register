<div x-data="{ show_new: false, show_update: false }">
    <div
        ...
        <div
            ...
            <div class="flex items-center justify-between ">
                <div>
                    <h3 class="text-lg font-semibold text-slate-800 dark:text-gray-200">Gépjárművek</h3>
                </div>
                ...
                <thead>
                    <tr>
                        <th
                            ...
                            <flux:input wire:model.live="filter_licence_plate" size="sm" placeholder="Rendszám" />
                        </th>
                        <th
                            class="p-2 transition-colors border-y border-slate-200 bg-slate-50 hover:bg-slate-100 dark:bg-neutral-900 dark:hover:bg-neutral-800">
                            <flux:input wire:model.live="filter_type" size="sm" placeholder="Típus" />
                        </th>
                        <th
                            class="p-2 transition-colors border-y border-slate-200 bg-slate-50 hover:bg-slate-100 dark:bg-neutral-900 dark:hover:bg-neutral-800">
                            <flux:input wire:model.live="filter_vin" size="sm" placeholder="Alvázszám" />
                        </th>
                        <th
                            class="p-2 transition-colors border-y border-slate-200 bg-slate-50 hover:bg-slate-100 dark:bg-neutral-900 dark:hover:bg-neutral-800">
                        ...
                        <th
                            class="p-2 transition-colors border-y border-slate-200 bg-slate-50 hover:bg-slate-100 dark:bg-neutral-900 dark:hover:bg-neutral-800">
                            <flux:button @click.prevent="$wire['reset_vehicles_filter']()" variant="primary"
                ...
                <tbody>
                    @foreach ($vehicles as $vehicle)
                        <tr>
                            ...
                            <td class="p-2 border-b border-slate-200">
                                <div class="w-max">
                                    @if ($vehicle->status->technical_name == 'active')
                                        <div
                                            ...
                                            <span class="">{{ $vehicle->status->display_name }}</span>
                                        </div>
                                    @else
                                        <div
                            ...
                            <td class="p-2 border-b border-slate-200">
                                <button @click.prevent="show_update = true; update_vehicle_id = {{ $vehicle->id }}"
    ...
    <livewire:admin.components.vehicles-modal :$statuses />
</div>
