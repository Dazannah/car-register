<div x-data="{ {{-- show_new: false, --}} show_update: false }">
            ...
            <table class="w-full mt-4 text-left table-auto min-w-max">
                <thead>
                    <tr>
                        ...
                        <th
                            class="p-2 transition-colors border-y border-slate-200 bg-slate-50 hover:bg-slate-100 dark:bg-neutral-900 dark:hover:bg-neutral-800">
                            <flux:select wire:model.live="filter_reservation_type_id" size="sm">
                                <flux:select.option value="">Válassz típust</flux:select.option>
                                @foreach ($reservation_types as $reservation_type)
                                    <flux:select.option value="{{ $reservation_type->id }}">
                                        {{ $reservation_type->display_name }}
                                    </flux:select.option>
                                @endforeach
                            </flux:select>
                        </th>
                ...
                <tbody>
                    @foreach ($trips as $trip)
                        <tr>
                            ...
                            <td class="p-2 border-b border-slate-200">
                                <div class="flex flex-col">
                                    @if ($trip->is_closed || $trip->reservationType->technical_name == 'pre')
                                        <input class="text-slate-700 dark:text-gray-200" type="datetime-local"
                                            name="return-time"
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="flex flex-col p-1 bg-white dark:bg-neutral-900">
                                        <p class="text-sm font-semibold text-slate-700 dark:text-gray-200">
                                            ...
                                            {{ $trip->reservationType->display_name }}
                                        </p>
                                    </div>
                                </div>
                            </td>
    ...
    <livewire:admin.components.manage-history-modal :$reservation_types />
</div>
